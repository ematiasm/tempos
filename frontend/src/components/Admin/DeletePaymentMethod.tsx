...
const DeletePaymentMethod = ({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      showSuccessToast(t("admin.finance.paymentMethodDeleted"))
      setIsOpen(false)
      onSuccess()
    },
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
        <Trash2 />
        {t("admin.finance.deletePaymentMethod")}
      </DropdownMenuItem>
      <DialogContent className="sm:max-w-md">
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogHeader>
            <DialogTitle>{t("admin.finance.deletePaymentMethod")}</DialogTitle>
            <DialogDescription>
              {t("admin.finance.deletePaymentMethodConfirm", {
