...
const DocumentDetailSheet = ({
  ...
  const convertMutation = useMutation({
    ...
    onSuccess: (invoice) => {
      showSuccessToast(t("documents.invoiceIssued", { numero: invoice.numero }))
  ...
  const {
    ...
    queryKey: ["payments-allocations", document?.id],
    enabled: open && isReceipt,
  })
  const {
    data: incomingAllocations,
    isLoading: incomingAllocationsLoading,
    isError: incomingAllocationsError,
  } = useQuery({
    queryFn: () =>
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-xl overflow-y-auto">
        ...
        <div className="flex flex-col gap-4 py-4">
          <div>
            <h4 className="text-sm font-medium mb-2">{t("documents.lines")}</h4>
          ...
          {!isReceipt && (
            <div>
              <h4 className="text-sm font-medium mb-2">
                {t("documents.receiptAllocationsTitle")}
              </h4>
              {incomingAllocationsLoading ? (
                <Skeleton className="h-24 w-full" />
              ) : incomingAllocationsError ? null : (incomingAllocations ?? [])
                  .length === 0 ? (
                <p className="py-4 text-center text-sm text-muted-foreground">
                  {t("documents.noReceiptAllocations")}
                </p>
              ) : (
                <ul className="divide-y rounded border">
                  {(incomingAllocations ?? []).map((allocation) => (
                    <li
          ...
          {isReceipt && (
            <div>
              <h4 className="text-sm font-medium mb-2">
                {t("documents.allocationsTitle")}
              </h4>
              {allocationsLoading ? (
                <Skeleton className="h-24 w-full" />
              ) : allocationsError ? null : (allocations ?? []).length === 0 ? (
                <p className="py-4 text-center text-sm text-muted-foreground">
                  {t("documents.noAllocations")}
                </p>
              ) : (
                <ul className="divide-y rounded border">
